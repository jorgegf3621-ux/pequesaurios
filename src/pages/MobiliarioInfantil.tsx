...
type MobCfg = {
  ...
  silla1_titulo: string;
  silla1_desc: string;
  silla1_img: string | null;
  silla2_titulo: string;
  silla2_desc: string;
  silla2_img: string | null;
};

const DEFAULTS: MobCfg = {
  badge: "Favorito Mamás",
  titulo: "Mesita de Madera Blanca",
  ...
  img_hero: null,
  silla1_titulo: "Sillas Pasteles",
  silla1_desc: "Sillas de colores suaves: rosa, lila, menta y amarillo. Perfectas para cualquier temática.",
  silla1_img: null,
  silla2_titulo: "Sillas Conejito & Arcoíris",
  silla2_desc: "Figuras blancas en forma de conejito y arcoíris. Adorables para fiestas con temáticas tiernas y coloridas.",
  silla2_img: null,
};

const MobiliarioInfantil = () => {
  const navigate = useNavigate();
  const [cfg, setCfg] = useState<MobCfg>(DEFAULTS);

  useEffect(() => {
    (async () => {
      const { data } = await (supabase as any).from("mobiliario_config").select("*").eq("id", 1).single();
      if (data) {
        const features = Array.isArray(data.features) ? data.features : DEFAULTS.features;
        setCfg({ ...DEFAULTS, ...data, features });
      }
    })();
  }, []);

  const heroSrc = cfg.img_hero ?? mesitaImg;
  const s1Src = cfg.silla1_img ?? mesitaBlancaImg;
  const s2Src = cfg.silla2_img ?? mesitaBlancaImg;
    ...
    <div className="min-h-screen">
      ...
      <section className="relative bg-gradient-to-b from-pink-50 via-purple-50 to-white py-20 px-4 text-center overflow-hidden">
        <div className="container mx-auto max-w-3xl relative z-10">
          ...
          <Button variant="hero" size="lg" onClick={() => navigate("/cotizador?paquete=mesa-blanca")}>
            Cotizar
          </Button>
        </div>
      </section>
      ...
      <section className="container mx-auto px-4 py-16 max-w-4xl">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div className="rounded-3xl overflow-hidden shadow-lg aspect-[4/3]">
            <img src={heroSrc} alt="Mesita infantil" className="w-full h-full object-cover" />
          </div>
          ...
          <div>
            ...
            <h2 className="font-heading text-3xl font-bold mb-2">{cfg.titulo}</h2>
            <p className="text-muted-foreground mb-6">{cfg.descripcion}</p>
            ...
            <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4 mb-6">
              <p className="font-heading text-4xl font-bold text-primary">${cfg.precio.toLocaleString()}</p>
              <p className="text-sm text-muted-foreground">MXN por mesa</p>
            </div>

            <div className="flex flex-col sm:flex-row gap-3">
              <Button variant="hero" size="lg" className="flex-1" onClick={() => navigate("/cotizador?paquete=mesa-blanca")}>
                Cotizar este servicio
              </Button>
              <Button variant="outline" size="lg" className="flex-1" asChild>
                <a href="https://wa.me/528180540369?text=%C2%A1Hola!%20Me%20interesa%20rentar%20el%20Mobiliario%20Infantil%20(mesita%20blanca)"
      ...
      <section className="bg-gradient-to-b from-purple-50 to-white py-14 px-4">
        <div className="container mx-auto max-w-3xl">
          <h2 className="font-heading text-2xl font-bold text-center mb-2">Elige el estilo de sillas</h2>
          <p className="text-muted-foreground text-center mb-10 text-sm">
            Ambas opciones están incluidas en el mismo precio de ${cfg.precio.toLocaleString()}
          </p>
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <div className="bg-white border-2 border-pink-200 rounded-3xl p-6 shadow-sm flex flex-col items-center text-center gap-4">
              <div className="w-full aspect-square rounded-2xl overflow-hidden bg-pink-50 flex items-center justify-center">
                <img src={s1Src} alt={cfg.silla1_titulo} className="w-full h-full object-contain p-4" />
              </div>
              <div>
                <h3 className="font-heading font-bold text-lg mb-1">{cfg.silla1_titulo}</h3>
                <p className="text-sm text-muted-foreground">{cfg.silla1_desc}</p>
              </div>
            </div>
            ...
            <div className="bg-white border-2 border-purple-200 rounded-3xl p-6 shadow-sm flex flex-col items-center text-center gap-4">
              <div className="w-full aspect-square rounded-2xl overflow-hidden bg-purple-50 flex items-center justify-center">
                <img src={s2Src} alt={cfg.silla2_titulo} className="w-full h-full object-contain p-4" />
              </div>
              <div>
                <h3 className="font-heading font-bold text-lg mb-1">{cfg.silla2_titulo}</h3>
                <p className="text-sm text-muted-foreground">{cfg.silla2_desc}</p>
              </div>
            </div>
          </div>
      ...
      <section className="bg-gradient-to-r from-pink-100 to-purple-100 py-14 px-4 text-center">
        <h2 className="font-heading text-2xl font-bold mb-3">¿Lista para apartar la mesita?</h2>
