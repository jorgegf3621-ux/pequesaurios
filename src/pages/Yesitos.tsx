...
const kits = [
  {
    ...
    emoji: "🐣",
    precio: 20,
    detalle: "1 yesito y 1 pintura",
    items: [
      "1 yesito de personaje",
      "1 pintura",
      "Pincel",
      "Sticker sorpresa",
      "Bolsa + etiqueta personalizada",
    ],
    popular: false,
  ...
  {
    id: "yesito-intermedio",
    nombre: "Intermedio",
    emoji: "🌈",
    precio: 25,
    detalle: "2 yesitos y 2 pinturas",
    items: [
      "2 yesitos de personajes",
      "2 pinturas",
      "Pincel",
      "Sticker sorpresa",
      "Bolsa + etiqueta personalizada",
  ...
  },
  {
    ...
    emoji: "🎉",
    precio: 30,
    detalle: "3 yesitos y 3 pinturas",
    items: [
      "3 yesitos de personajes",
      "3 pinturas",
      "Pincel",
      "Sticker sorpresa",
      "Bolsa + etiqueta personalizada",
  ...
  <div className="min-h-screen">
    ...
    <section className="bg-gradient-to-b from-yellow-50 via-pink-50 to-white py-20 px-4 text-center">
      <div className="container mx-auto max-w-3xl">
        ...
        <p className="text-lg text-muted-foreground max-w-xl mx-auto">
          Los peques pintan su figura y se llevan un recuerdo único.
          Diseño de etiqueta personalizada según la temática de tu fiesta.
        </p>
      </div>
    </section>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {kits.map((kit) => (
          ...
          >
            ...
            <p className="text-3xl mb-2">{kit.emoji}</p>
            <h3 className="font-heading text-xl font-bold mb-0.5">{kit.nombre}</h3>
            <p className="text-xs text-muted-foreground italic mb-3">({kit.detalle})</p>
            ...
            <Button variant={kit.btn} className="w-full" asChild>
              <a
                href={`https://wa.me/528180540369?text=${encodeURIComponent(
                  `¡Hola! Me interesa el Kit Yesitos ${kit.nombre} ($${kit.precio} MXN c/u)`
                )}`}
                target="_blank"
    ...
    <section className="bg-gradient-to-b from-pink-50 to-white py-14 px-4">
      <div className="container mx-auto max-w-3xl">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {personajes.map((p) => (
            <span key={p} className="bg-white border border-border rounded-full px-3 py-1 text-sm font-medium shadow-sm">
              {p}
            </span>
          ))}
        </div>
    ...
    <section className="container mx-auto px-4 py-14 max-w-2xl">
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        {entrega.map((punto) => (
          <div key={punto} className="flex items-center gap-3 bg-card border border-border rounded-xl p-3 shadow-sm">
